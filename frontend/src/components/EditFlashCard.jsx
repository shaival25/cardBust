...
function EditFlashcardPack () {
    ...
    const updateProcess = async () => {
      ...
      const updatePromises = flashCards.map(async card => {
        if (!card.question || !card.answer) {
          throw new Error(`Please fill out all fields for flashcard ${card.id}`)
        }
        return axios.put(
          `${import.meta.env.VITE_BACKEND_URI}/api/v1/flashcard/${card.id}`,
          {
  ...
  return (
    <div className='max-w-7xl mx-auto bg-gray-500 min-h-screen p-6'>
      ...
      <main className='space-y-8'>
        ...
        <button
          ...
          className='w-full text-gray-900 bg-green-700 hover:bg-green-600 text-gray-700 font-semibold py-3 rounded-lg transition duration-300'
        >
          + Add Flashcard
        </button>
      </main>
